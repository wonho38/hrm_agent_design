...
class Guardrail:
    ...
    def __init__(self, include_readability_report: bool = True):
        """
        Initialize base Guardrail with readability functionality.
        ...
        self.readability_checker = ReadabilityChecker() if include_readability_report else None
    ...
    def post_guard(self, raw_output: str) -> str:
        """
        Validate and possibly redact model output with optional readability analysis.
        ...
        """
        if not self.include_readability_report or not self.readability_checker:
            return raw_output
        ...
        try:
            ...
            print(f"[{self.__class__.__name__}] 가독성 검사 중 오류 발생: {e}")
    ...
    def set_readability_enabled(self, enabled: bool) -> None:
        ...
        if enabled and not self.readability_checker:
            self.readability_checker = ReadabilityChecker()
        elif not enabled:
            self.readability_checker = None
...
class OperationHistoryGuardrail(Guardrail):
    ...
    def __init__(self, include_readability_report: bool = True):
        ...
        Args:
            include_readability_report: Whether to include readability analysis in post_guard
        """
        super().__init__(include_readability_report)
...
class GuideGuardrail(Guardrail):
    """Guardrail specifically for guide generation validation and readability checking."""

    def __init__(self, include_readability_report: bool = True):
        """
        Initialize GuideGuardrail.
    ...
    def pre_guard(self, guide_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Pre-process guide data (currently no validation needed).
        
        Args:
            guide_data: Dictionary containing guide data
            
        Returns:
            Dict[str, Any]: Validated guide data
        """
        return guide_data


class ImageAnalysisGuardrail(Guardrail):
    """Guardrail for image analysis input validation and output processing."""

    def __init__(self, include_readability_report: bool = True):
        """
        Initialize ImageAnalysisGuardrail.
        
        Args:
            include_readability_report: Whether to include readability analysis
        """
        super().__init__(include_readability_report)

    def pre_guard(self, analysis_request: Dict[str, Any]) -> Dict[str, Any]:
        """
        Validate image analysis request before processing.
        
        Args:
            analysis_request: Dictionary containing image data and analysis parameters
            
        Returns:
            Dict[str, Any]: Validated analysis request
            
        Raises:
            GuardrailException: If image data is invalid or missing
        """
    ...
    def post_guard(self, analysis_result: str) -> str:
        """
        Post-process analysis result with readability check.
        
        Args:
            analysis_result: Raw analysis result text
            
        Returns:
            str: Processed result with optional readability report
        """
        # Inherited from base Guardrail class
        return super().post_guard(analysis_result)
